...
export default class Driver {
    ...
    search(passenger, ticks, globals){
        ...
        else{ // if no passenger found
            this.passenger = passenger;
            
            if (this.destination === null || this.distanceToTravel === 0){ // if no destination set or destination reached
                this.time += 1; 
                // let setDestinationRandom = Math.random();
                if (this.time > this.framesToMove){ // if time spent searching is more than framesToMove
                    this.destination = this.generateRandomCoordBelow(this.currentLocation,this.moveRadius);
                    this.speedLs.push(0);
                }
                return false; //is not moving
            }

            else{ // if destination is set and not reached
                ...
                else{ // if destination is < distance covered in 1 tick
    ...
    pickUp(ticks,globals){
        ...
        if (minTime === 1 && this.distanceToTravel !== speed){ //NOTE: like 4km -> 2.66km etc (no remainder)
            this.distance += speed
            this.distanceToTravel -= speed
            // return false;
        }
    ...
    generateRandomCoord() { //NOTE: generate random coordinate within singapore
    ...
    resetTripVariables(){ //NOTE: reset trip variables
        this.tripDistance = 0;
        this.tripTime = 0;
        this.tripFuelCost = 0;
        this.tripFare = 0;
    ...
    deg2rad(deg) { //NOTE: convert degree to radian
      return deg * (Math.PI / 180)
    }

    getDistance(start, end) { // NOTE: start and end are [lon, lat]
        let lat1 = start[1];
        let lon1 = start[0];
        let lat2 = end[1];
        let lon2 = end[0];
      ...
      generateRandomCoordBelow(startCoord,distance){
        ...
        console.log('Found randomCoord', randomCoord, 'distance', distanceToRandomCoord)
        return randomCoord;
      }
